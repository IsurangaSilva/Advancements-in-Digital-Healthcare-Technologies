...
collection = mongo_connection.get_collection("voice-emotion-60minaggregates")

class VoiceEmotionAggregator60:
    def __init__(self, interval_seconds=60, emotion_file=None, session_file=None):
        
        db_dir = os.path.join("db", "Audio")
    
        self.emotion_file = emotion_file or os.path.join(db_dir, "voice_session_summary.json")
        self.session_file = session_file or os.path.join(db_dir, "voice_session_summary_60.json")
    ...
    def aggregate(self):
        print(f"[{datetime.now().isoformat()}] Voice 60min session aggregation started.")
        ...
        with file_lock:
            ...
            print(f"Voice session aggregated and saved: {session_aggregate60}")
...
if __name__ == "__main__":
    aggregator = VoiceEmotionAggregator60()
    aggregator.run()
