...
def start_personalization(avg_embedding):
    # This function is retained in case you want to run personalization manually.
    root = tk.Tk()
    root.withdraw()
...
def start_reference_capture():
    ...
    def on_capture_complete(avg_embedding):
        root.destroy()
        # Directly launch the session app using the captured average embedding.
        launch_session_app(avg_embedding)
...
if __name__ == "__main__":
    ...
    if os.path.exists(ref_emb_path):
        print("Reference embedding found. Skipping capture and personalization...")
