const mongoose = require("mongoose");
const connection = mongoose.connection.useDb("emotionDB");

const voiceAggregation60Schema = new mongoose.Schema(
  {
    _id: mongoose.Schema.Types.ObjectId,
    timestamp: {
      type: String,
      required: true,
    },
    session_aggregate: {
      angry: { type: Number, required: true },
      fear: { type: Number, required: true },
      happy: { type: Number, required: true },
      neutral: { type: Number, required: true },
      sad: { type: Number, required: true },
      surprise: { type: Number, required: true },
    },
  },
  { timestamps: true }
);

const Voice_Emotion_60Aggregate = connection.model(
  "voice-emotion-60minaggregate",voiceAggregation60Schema
);

module.exports = Voice_Emotion_60Aggregate;
